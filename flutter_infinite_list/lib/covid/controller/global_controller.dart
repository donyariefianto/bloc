...
import '../models/detail_country_entity.dart';
import '../models/global_entity.dart';
...
class GlobalController extends GetxController {
  ...
  final Rx<CountryEntity?> _countryEntity = CountryEntity().obs;
  final Rx<DetailCountryEntity?> _detailCountryEntity =
      DetailCountryEntity().obs;
  ...
  void _fetchGlobalData() async {
    try {
      ...
      confirmedTotal.value =
          globalEntity.value?.confirmed?.value?.toDouble() ?? 0.0;
      recoveredTotal.value =
          globalEntity.value?.recovered?.value?.toDouble() ?? 0.0;
  ...
  void _fetchCountrySelected() async {
    try {
      Map<String, dynamic> data = await (covidDataSource
